...
{
	/// <summary>
	/// Contains properties for the various options available. For example the <see cref="StopRuleProcessingOnFirstFailure"/> property determines
	/// whether all rules in the collection will be processed for a <see cref="WorkAction"/> child class, or if rule validation stops on the first
	/// failed rule encountered.
	/// </summary>
	public class WorkActionConfiguration
	{
		...
		/// <summary>
		/// Constructor overload accepting a <see cref="TransactionScope" /> object to use for chaining multiple <see cref="IWorkAction" /> calls
		/// together and there is a need for transactions spanning those actions.
		/// </summary>
